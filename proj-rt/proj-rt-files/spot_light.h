...
public:
    ...
    vec3 Emitted_Light(const vec3& vector_to_light) const
    {
        ...
        double top = pow(min_cos_angle, falloff_exponent);
        vec3 numberator = top * color * brightness;
        double denominator = 4*pi*vector_to_light.magnitude_squared();
        vec3 lSpot = numberator / denominator;
        return lSpot;
    }
};
#endif
